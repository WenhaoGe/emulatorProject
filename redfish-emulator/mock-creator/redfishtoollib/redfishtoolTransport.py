...
class RfTransport():
    ...
    def __init__(self):
        ...
        self.elapsed=None
        self.isFImanaged = False

        requests.packages.urllib3.disable_warnings()

    def setFImanaged(self):
        from .sshClient import SSHClient
        self.ssh_client = SSHClient()
        self.ssh_client.connect(self.rhost, port=22,
                                username=self.user,
                                password=self.password)
        self.isFImanaged = True
    ...
    def getVersionsAndSetRootPath(self,rft,forceCheckProtocolVer=False):
        ...
        for attempt in range(0,rft.waitNum):
            try:
                ...
                if rft.isFImanaged:
                    get_method = rft.ssh_client.get
                t1=time.time()
                r = get_method(url, headers=hdrs, verify=False, timeout=(rft.waitTime,rft.timeout))  # GET ^/redfish
                t2=time.time()
                rft.elapsed = t2 - t1
                # print request headers
                if not rft.isFImanaged:
                ...
                #priet the response status (-ssss)
    ...
    def rftSendRecvRequest( rft, apiType, method, baseUrl, relPath=None, data=None, jsonData=True,  prop=None,
        ...
        for attempt in range(0,rft.MaxNextLinks):
            try:
                ...
                if rft.isFImanaged:
                    request_method = rft.ssh_client.request
                t1=time.time()
                r = request_method(method, url, headers=hdrs, auth=authType, verify=verify, data=reqData,
                ...
                # this exception needed as eequests is not catching socket timeouts
                #  especially "connection refused" eg web server not started
                # issue: https://github.com/kennethreitz/requests/issues/1236
                # Nothing timed out.  this is a connect error. So wait and retry
                rft.printVerbose(5,"Tranport: socket.error,  wait and try again")
